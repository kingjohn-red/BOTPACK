...
(function () {
  var any, bind, emit, escape, expand, parse, plugmaticListener, traffic;
  ...
  emit = function emit($item, item) {
    var dialog, markup, render, trouble;
    ...
    render = function render(data) {
      ...
      format = function format(markup, plugin, dependencies) {
        var j, len, months, name, ref, result, status;
        name = plugin.plugin;
      ...
      report = function report(markup, plugins, dependencies) {
        ...
        result = function () {
          var j, len, results;
          results = [];
          for (index = j = 0, len = inventory.length; j < len; index = ++j) {
      ...
      showdetail = function showdetail(e) {
        ...
        return detail(name, function (html) {
          var context, pageKey, plugmaticDialog;
          ...
          if (column === 'status') {
            ...
            if (plugmaticDialog.location.pathname !== '/plugins/plugmatic/dialog/') {
              return plugmaticDialog.addEventListener('load', function (event) {
                return plugmaticDialog.postMessage({
                  column: column,
                  title: "".concat(name, " plugin ").concat(column),
                  body: html || '',
                  pageKey: pageKey,
                  context: context
                }, window.origin);
  ...
  plugmaticListener = function plugmaticListener(event) {
    var $page, action, context, data, keepLineup, pageKey, title;
    ...
    switch (action) {
      case 'doInternalLink':
        var _data = data;
        var _data$keepLineup = _data.keepLineup;
        keepLineup = _data$keepLineup === void 0 ? false : _data$keepLineup;
        var _data$pageKey = _data.pageKey;
        pageKey = _data$pageKey === void 0 ? null : _data$pageKey;
        var _data$title = _data.title;
        title = _data$title === void 0 ? null : _data$title;
        var _data$context = _data.context;
        context = _data$context === void 0 ? null : _data$context;
        $page = null;
        if (pageKey !== null) {
          $page = keepLineup ? null : $('.page').filter(function (i, el) {
            return $(el).data('key') === pageKey;
          });
        }
